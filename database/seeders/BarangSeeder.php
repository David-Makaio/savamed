...
class BarangSeeder extends Seeder
{
    ...
    {
        // Fetch the specific pharmacy we want to seed items for
        $pharmacy = Apotek::where('nomor_lisensi', 'S0B-2025-TEST')->first();

        if (!$pharmacy) {
            $this->command->error('Pharmacy not found! Run ApotekSeeder first.');
            return;
        }
        $sampleMedicines = [
            [
        ...
        foreach ($sampleMedicines as $item) {
            // Create Category scoped to this pharmacy
            $kategori = Kategori::firstOrCreate([
                'nama_kategori' => trim($item['category']),
