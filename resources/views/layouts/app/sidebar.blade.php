...
<body
    class="min-h-screen bg-gradient-to-br from-primary-50 via-white to-teal-50 dark:from-brand-darker dark:via-slate-900 dark:to-slate-800 flex flex-col lg:flex-row transition-colors duration-300">
    <!-- Sidebar for Desktop -->
    <nav
        ...
        <div class="flex items-center gap-3 px-6 py-4">
            ...
            <span class="text-lg font-medium text-primary-900 dark:text-white">{{ config('app.name', 'Laravel') }}</span>
        </div>
                ...
                <a href="{{ route('dashboard') }}" wire:navigate
                    ...
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 12l2-3m0 0l7-4 7 4M5 9v10a1 1 0 001 1h12a1 1 0 001-1V9m-9 4v4m-4-4v4m8-4v4"></path>
                    </svg>
                    {{ __('Dashboard') }}
                </a>
    ...
    <div class="flex-1 flex flex-col">
        ...
        <nav id="mobile-sidebar"
            class="hidden lg:hidden border-b border-primary-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-4 flex flex-col gap-4">
            <div class="space-y-2">
                <h3 class="px-2 text-xs font-semibold text-primary-600 dark:text-teal-300 uppercase tracking-wider">
                    {{ __('Platform') }}</h3>
                <a href="{{ route('dashboard') }}" wire:navigate
                    class="flex items-center gap-3 px-3 py-2.5 text-sm rounded-lg transition-all {{ request()->routeIs('dashboard') ? 'bg-teal-100 dark:bg-teal-900/30 text-primary-900 dark:text-teal-300 font-semibold shadow-sm' : 'text-primary-700 dark:text-slate-300' }} rounded-lg">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 12l2-3m0 0l7-4 7 4M5 9v10a1 1 0 001 1h12a1 1 0 001-1V9m-9 4v4m-4-4v4m8-4v4"></path>
                    </svg>
        ...
        @if (session('message'))
            <div class="fixed top-4 right-4 bg-teal-50 dark:bg-teal-900/20 border border-teal-300 dark:border-teal-700/50 text-teal-900 dark:text-teal-200 px-4 py-3 rounded-lg shadow-lg max-w-sm z-50 backdrop-blur-sm"
                role="alert">
                <div class="flex">
